...
float AntialiasedCutoff(float distance){
  float distanceChange = fwidth(distance) * 1;
...
float CalcAlphaInverted(float2 samplePosition, float2 size, float radius, float thickness){
  float2 samplePositionTranslated = (samplePosition - .5) * size;
    
  // Compute distance for the outer boundary
  float distToOuterRect = roundedRectangle(samplePositionTranslated, radius * .5, size * .5);

  // Compute distance for the inner boundary by reducing the size by the thickness
  float distToInnerRect = roundedRectangle(samplePositionTranslated, (radius-thickness) * .5, (size-thickness) * .5);

  // The alpha should be 1.0 in the region between distToInnerRect and distToOuterRect and 0.0 elsewhere
  // Use smoothstep for outer and 1.0 - smoothstep for inner to invert and then multiply the two
  return AntialiasedCutoff(distToOuterRect) * (1.0 - AntialiasedCutoff(distToInnerRect));
}
