...
namespace Modio.API
{
    public static partial class ModioAPI
    {
        ...
        {
            /// <summary>Get all mod's the _authenticated user_ is subscribed to. Successful request will return an array of [Mod Objects](#get-mods-2). We recommended reading the [filtering documentation](#filtering) to return only the records you want.</summary>
            /// <param name="filter">Filter to apply to the request.</param>
            internal static async Task<(Error error, JToken modObjects)> GetUserSubscriptionsAsJToken(
            GetUserSubscriptionsFilter filter
            ) {
                if (!IsInitialized()) return (new Error(ErrorCode.API_NOT_INITIALIZED), null);

                using var request = ModioAPIRequest.New($"/me/subscribed", ModioAPIRequestMethod.Get);
            ...
            /// <summary>Get all mod's the _authenticated user_ is subscribed to. Successful request will return an array of [Mod Objects](#get-mods-2). We recommended reading the [filtering documentation](#filtering) to return only the records you want.</summary>
            /// <param name="filter">Filter to apply to the request.</param>
            internal static async Task<(Error error, Pagination<ModObject[]>? modObjects)> GetUserSubscriptions(
            GetUserSubscriptionsFilter filter
            ) {
                if (!IsInitialized()) return (new Error(ErrorCode.API_NOT_INITIALIZED), null);
...
#region Filter
            ...
            public static GetUserSubscriptionsFilter FilterGetUserSubscriptions(
                int pageIndex = 0,
                int pageSize = 100
            ) 
            => new GetUserSubscriptionsFilter(
                pageIndex, 
                pageSize
            );
            
            /// <summary>
            /// Filter for GetUserSubscriptions, see <see cref="Me.FilterGetUserSubscriptions"/>
            ...
            {
                internal GetUserSubscriptionsFilter(
                    int pageIndex,
                    int pageSize
