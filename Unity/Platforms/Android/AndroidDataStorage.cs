...
namespace Modio.Unity.Platforms.Android
{
    public class AndroidDataStorage : BaseDataStorage
    {
        ...
        protected override void MigrateLegacyModInstalls()
        {
            ...
            MigrateLegacyModInstalls(Path.Combine(Application.persistentDataPath, "Installed"));
            MigrateLegacyModInstalls(Path.Combine(Application.persistentDataPath, "mods"));

            // The below are smaller with minimal impact, so don't need to be handled as delicately as mod installs
            try
            {
                if (Directory.Exists(Path.Combine(Application.persistentDataPath, "ImageCache")))
                    Directory.Delete(Path.Combine(Application.persistentDataPath, "ImageCache"), true);

                if (Directory.Exists(Path.Combine(Application.persistentDataPath, "Temp")))
                    Directory.Delete(Path.Combine(Application.persistentDataPath, "Temp"), true);
            }
            catch (Exception e)
            {
